...
def load_and_finetune_model_dense(model, char, lr_value):
    """Loads a pre-trained model and uses it for transferlearning.

    This function takes a pre-trained model and fine-tunes it for handwriting recognition
    by freezing all layers except the dense output layer, replacing the output layer with
    a new dense layer suitable for the desired number of characters.

    Args:
        model: The pre-trained Keras model. Default: model9v3_xl
        char (int): The total number of occurring characters
        lr_value (float): The learning rate value.
    ...
    output = CTCLayer(name="ctc_loss")(model.input[1], x) 
...
# Everything below this point can be ignored, since it doesn't hold any further use 
...
def build_model9v4(img_width, img_height, char, lr_value):
    input_img = keras.Input(shape=(img_width, img_height, 1), name="image")
    labels = keras.layers.Input(name="label", shape=(None,))
...
def build_model9v3_augmented(img_width, img_height, char, lr_value):
    input_img = keras.Input(shape=(img_width, img_height, 1), name="image")
    labels = keras.layers.Input(name="label", shape=(None,))
...
def build_model9v3_random_seed(img_width, img_height, char, learning_rate_value, seed=None):
    if seed is not None:
        np.random.seed(seed)
...
def load_and_finetune_model_full(model, lr_value):
    for layer in model.layers:
